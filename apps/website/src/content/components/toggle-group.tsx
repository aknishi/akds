...
export const toggleGroup: ComponentEntry = {
  ...
  accessibilityNotes: [
    'Renders as an ARIA radiogroup — each ToggleButton child becomes role="radio" with aria-checked, so only one option is ever selected.',
    'Supports full keyboard navigation: Arrow keys move between options, Home/End jump to the first/last enabled option.',
    "Grouped ToggleButton children ignore their own disabled/size props unless set — the group's size and disabled propagate to every child.",
  ],
  props: [
    ...
    { name: 'disabled', type: 'boolean', default: 'false', description: 'When true, disables all child ToggleButton components.' },
    { name: 'size', type: "'sm' | 'md' | 'lg'", default: "'md'", description: 'Sets the size applied to all child ToggleButton components.' },
    { name: 'children', type: 'React.ReactNode', description: 'ToggleButton components to render inside the group. Required.' },
  ],
  doDont: [
    { do: 'Use ToggleGroup for a small, fixed set of mutually exclusive options (view mode, time range).', dont: "Don't use ToggleGroup as a substitute for Tabs — it selects a value, it doesn't switch page content." },
  ],
